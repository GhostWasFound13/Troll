...
$let[id;$sendMessage[{newEmbed:{author:Adding to playlist:$getVar[loademoji]} {timestamp} {color:$getVar[color]}};yes]]
$endif
$let[message;$replaceText[$replaceText[$message;";'];:;#COLON#]]
...
$let[message;$playTrack[$replaceText[$replaceText[$checkContains[$get[url];youtube.com];true;youtube];false;$replaceText[$replaceText[$checkContains[$get[url];soundcloud.com];true;soundcloud];false;$replaceText[$replaceText[$checkContains[$get[url];open.spotify.com];true;spotify];false;$replaceText[$replaceText[$checkCondition[$checkContains[$get[url];youtube.com;soundcloud.com;open.spotify.com]==$isValidLink[$get[url]]];true;youtube];false;url]]]];$get[url]]]
$onlyIf[$hasPlayer!=false;]
$onlyIf[$checkContains[$get[url];open.spotify.com/artist;open.spotify.com/album;open.spotify.com/episode]!=true;Not support]
$if[$voiceID[$clientID]==]
...
$onlyIf[$truncate[$message[1]]>0;Your playlist only has **$numberSeparator[$sub[$getGlobalUserVar[playlistusercount];1]]** song.]
...
$let[seconduser;$charCount[$replaceText[$getGlobalUserVar[playlistuser;$interactionData[author.id]];:;#colon#]]]
...
},
{
  ...
  type: "interaction",
  prototype: "button",
  code: `$interactionReply[;;;{file:#RIGHT_BRACKET#\n"author_playlist"#COLON# "$userTag[$interactionData[author.id]]-$interactionData[author.id]",\n"playlist_count"#COLON# "$sub[$getGlobalUserVar[playlistusercount;$interactionData[author.id]];1]",\n"playlist_status"#COLON# #RIGHT_BRACKET#\n "auto_add"#COLON# "$getGlobalUserVar[playlistuserauto;$interactionData[author.id]]",\n "words"#COLON# "$getGlobalUserVar[playlistuserwords;$interactionData[author.id]]",\n "public"#COLON# "$getGlobalUserVar[playlistuserpublic;$interactionData[author.id]]"\n#LEFT_BRACKET#,\n"playlist"#COLON# #RIGHT_BRACKET#\n$cropText[$get[file];$charCount[$get[file]];2]\n #LEFT_BRACKET#\n#LEFT_BRACKET#:$username[$interactionData[author.id]]-$dateStamp.txt};;yes]
$onlyIf[$checkContains[$getEmbed[$channelID;$interactionData[message.id];1;thumbnail];$authorID]==true;{
...
$onlyIf[$checkContains[$getEmbed[$channelID;$interactionData[message.id];1;thumbnail];$authorID]==true;{
 ...
 code: `$deleteMessage[$interactionData[message.id]]
$wait[3s]
$interactionUpdate[;{newEmbed:{author:Canceled.} {color:$getVar[color]} {timestamp}}]
$onlyIf[$checkContains[$getEmbed[$channelID;$interactionData[message.id];1;thumbnail];$authorID]==true;{
 "content": "You cant use this button.",
 "ephemeral": "true",
 "options": {
